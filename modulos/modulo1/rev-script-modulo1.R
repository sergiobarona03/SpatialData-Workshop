...
## Solución:
# Cargar shapefile de Sedes Educativas georreferenciadas
sedes = st_read("vector_data\\shp_sedesSISE",
                layer = "SISE_WGS84")

# Cargar el shapefile de la ciudad de Cali
cali.shp <- st_read(dsn = "vector_data/Merge_Cali/",
                     layer = "Merge_Cali")

# Analizar la geometría de las comunas de Cali
cali.urbano = cali.shp %>% filter(!is.na(COMUNA))
plot(st_geometry(cali.urbano), border = "black")

# Selecciona las sedes educativas en Cali
cali.sedes = sedes[cali.shp,]

# Analizar la distribución de las sedes en Cali 
plot(st_geometry(cali.shp), border = "black")
plot(st_geometry(cali.sedes), col = "red", pch = 20,
     add = TRUE)

# Comuna 22 y sedes educativas 
comuna22 <- cali.urbano[cali.urbano$COMUNA == 22, ]
sedes.22 <- sedes.urbanas[comuna22, ]

# Dibujar las sedes de la comuna 22 por nivel
plot(st_geometry(comuna22), border = "gray30", col = "gray95",
     main = "Sedes educativas por nivel\nComunas 22", cex.main = 1.2)
plot(st_geometry(sedes.22[sedes.22$NIVEL == "PREESCOLAR", ]), col = "red", pch = 20, add = TRUE)
plot(st_geometry(sedes.22[sedes.22$NIVEL == "EDUCACIÓN BÁSICA PRIMARIA", ]), col = "green", pch = 20, add = TRUE)
plot(st_geometry(sedes.22[sedes.22$NIVEL == "EDUCACIÓN MEDIA", ]), col = "blue", pch = 20, add = TRUE)
legend("topright", legend = c("Preescolar", "Primaria", "Media"),
       col = c("red", "green", "blue"), pch = 20,
       title = "Nivel educativo", bty = "n", cex = 0.9)
