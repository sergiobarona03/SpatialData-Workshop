...
#### Solución:
ipm.mun = readxl::read_excel("vector_data\\CNPV_2018\\CNPV-2018_PobrezaMultidimensional_Municipal.xlsx",
                             sheet = 2)
ipm.mun$cod = as.numeric(ipm.mun$cod)  # sin esto falla el join
nbi.ipm = nbi.mun %>% left_join(ipm.mun[c("cod", "total")],
                                by = "cod") %>%
  rename(ipm = total)
ggplot() + geom_sf(data = nbi.ipm, aes(fill = ipm),
                   col = "black")
...
### Solución
barrios = st_read(dsn = "vector_data\\cali\\Barrios\\",
                  layer = "Barrios_WGS84")
sedes = st_read("vector_data\\shp_sedesSISE\\",
                layer = "SISE_WGS84")
sedes.cali = sedes[barrios,]

ggplot() +
  geom_sf(data= barrios, col = "black") +
  geom_sf(data = sedes.cali, aes(col = SECTOR,
                                 pch = SECTOR),
          size = 1) + theme_bw()
  

inter.sedes <- st_intersects(barrios, sedes.cali)
barrios$count <- lengths(inter.sedes)
summary(barrios$count)

barrios$count_cut = cut(barrios$count, 
                        breaks = c(0,1,3,6, 12, Inf),
                        include.lowest = T, right = F)
